        ...
        <div class="header-container md:border-b-2 border-white border-opacity-10 py-8 max-w-sm md:max-w-md lg:max-w-lg mx-auto w-11/12 flex justify-between items-center">
            <nav class="header__navbar lg:order-2">
                ...
                <div
                    id="menu"
                    v-show="isMenuOpen" 
                    class="navbar__categories-mobile focus:outline-none absolute w-screen bg-gray-50 left-0 pt-20 pb-8 top-22 rounded-b-lg lg:hidden"
                >
            ...
            <div class="header__cart lg:order-3 lg:w-2/12 lg:flex lg:justify-end">
...
export default {
    ...
    methods: {
        ...mapMutations('layout', ['SET_MENU_STATE','SET_CART_STATE']),
        setMenuState(value) {
            this.SET_MENU_STATE(value)
        },
        setCartState(value) {
            this.SET_CART_STATE(value)
        }
    },
    created() {
        if(process.client) {
